...
class GIF_spiking(models.Model):
    ...
    def __init__(self, params, spike_history, input_history,
        ...
        if pop_sizes is not None:
            ...
            self.pop_sizes = spike_history.shape
        # FIXME
        ...
        if not isinstance(self.s, Spiketrain) and not isinstance(self.s, Series):
            raise ValueError("Spike history must be an instance of sinn.Spiketrain or sinn.Series.")
        ...
        # if isinstance(self.s, Series):
        #     self.params = self.params._replace(
        #         **{name: params.__getattribute__(name) for name in params._fields})
            # self.params = self.params._replace(
            #     **{name: sinn.popterm.PopTermMicro(pop_sizes=params.__getattribute__('N'),
            #                                        values=params.__getattribute__(name),
            #                                        block_types=('Micro',))
            #        for name in params._fields})
        # else:
        #     self.params = self.params._replace(
        #         **{name: self.s.get_popterm(getattr(self.params, name)) for name in params._fields})
        self.params = self.params._replace(
            **{name: self.get_popterm(params.__getattribute__(name)) for name in params._fields})
    ...
    def get_popterm(self, values):
        if isinstance(values, sinn.popterm.PopTerm):
            return values
        else:
            # TODO: Find a way not to instantiate a PopTerm just for 'infer_block_type'
            dummy_popterm = sinn.popterm.PopTermMacro(self.pop_sizes, np.zeros(1), ('Macro',))
            block_types = dummy_popterm.infer_block_types(values.shape,
                                                          allow_plain=False)
            cls = sinn.popterm.PopTerm.BlockTypes[block_types[0]]
            return cls(self.pop_sizes, values, block_types)

    def set_connectivity(self, w, pop_slices):
        """
        ...
        self.conn_mats = [ [ (w[to_pop_slice, from_pop_slice])
                             for from_pop_slice in pop_slices ]
                           for to_pop_slice in pop_slices ]
